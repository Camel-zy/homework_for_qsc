...
func CreateObject(ctx context.Context, objectName string) (url *url.URL, err error) {
	...
	url, _, err = minioClient.PresignedPostPolicy(ctx, policy)
	if err != nil {
		logrus.Error(err)
		return
	}
...
func SealObject(ctx context.Context, uuid uuid.UUID) error {
	result := gormDb.Model(&PermanentObject{UUID: uuid}).Updates(&PermanentObject{Finished: true})
	return result.Error
}

func GetObject(ctx context.Context, uuid uuid.UUID) (presignedURL *url.URL, err error) {
	var permanentObject PermanentObject
	result := gormDb.Model(&PermanentObject{UUID: uuid}).First(&permanentObject)
	if result.Error != nil {
		logrus.Error(err)
		return nil, err
	}
